...
class EncDec(nn.Module):
    def __init__(
        ...
        self.nll = nn.CrossEntropyLoss(ignore_index=0, reduction="sum")  # TODO: why mean better?
        self.nll_wr = nn.CrossEntropyLoss(ignore_index=0, reduction="none")
        self.dim = dim
    ...
    def forward(self, inp, out, lens=None, per_instance=False):
        ...
        if per_instance:
            ...
            dec = dec.view(-1, len(self.tokenizer))
    ...
    def logprob_interleaved(self, inp, out, lens=None):
        ...
        if self.source_att:
            ...
            att_tokens = None
        return self.decoder.logprob(
            outbatch, rnn_state=sbatch, att_features=att_features, att_tokens=att_tokens
    ...
    def beam(self, inp, beam_size, lens=None):
        ...
        return self.decoder.beam(
            state, beam_size, 150, att_features=att_features, att_tokens=att_tokens
        )
    ...
    def sample_with_gumbel(self, inp, max_len, lens=None, temp=1.0, **kwargs):
        ...
        return self.decoder.sample_with_gumbel(
            state, max_len, att_features=att_features, att_tokens=att_tokens, temp=temp
        )
