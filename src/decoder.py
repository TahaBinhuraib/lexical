...
from .utils import trim, weight_top_p
...
class Decoder(nn.Module):
    def __init__(
        ...
        # modules
        self.embed = nn.Embedding(len(tokenizer), n_embed, 0)

        self.dropout_in = nn.Dropout(dropout)
        self.predict = nn.Linear(n_hidden, len(tokenizer))
        ...
        self.nll = nn.CrossEntropyLoss(reduction="none", ignore_index=0)
        self.nllreduce = nn.CrossEntropyLoss(ignore_index=0)
        self.copy_translation = None
        self.high_drop = nn.Dropout(FLAGS.highdropvalue)

    def step(
        self, decoder_state, att_features, att_tokens, att_masks, att_token_proj, self_att_proj
    ):
        # advance rnn
        ...
        if self.self_attention:
            ...
            att_masks = att_masks + ((decoder_state.tokens == 0).float(),)
            att_token_proj = att_token_proj + (self_att_proj,)

        # advance attention
        attended = [
            attention(features, hidden, mask)
            for attention, features, mask in zip(self.attention, att_features, att_masks)
        ]
        if len(attended) > 0:
        ...
        if self.copy:
            ...
            copy_weights = pred_probs[:, 3].unsqueeze(-1)
        ...
        # done
        return (pred_logits, comb_features, rnn_state, hidden, direct_logits, copy_logits, dists)
    ...
    def _make_projection(self, tokens):
        if self.copy:
            if type(self.copy_translation) == SoftAlign:
                ...
                proj = np.zeros(
                    (tokens.shape[0], tokens.shape[1], len(self.tokenizer)), dtype=np.int64
                )
    ...
    def get_mask(self, toks):
        if toks.dtype == torch.float32:
            mask = (torch.argmax(toks.detach(), dim=-1) == 0).float()
        else:
            mask = (toks == 0).float()
        return mask
    ...
    def forward(
        ...
        if att_features is None:
            ...
            assert isinstance(att_features, list) or isinstance(att_features, tuple)
            assert len(att_features) == len(att_tokens)
            assert len(self.attention) == len(att_features) + (1 if self.self_attention else 0)
            att_masks = tuple(self.get_mask(toks) for toks in att_tokens)
            att_token_proj = tuple(self._make_projection(toks) for toks in att_tokens)

        # init
        pred = None
        tokens, self_att_proj = token_picker(0, pred)
        feed = tokens.new_zeros(tokens.shape[0], self.n_hidden).float()
        hiddens = []
        all_tokens = [tokens]
        all_preds = []
        all_extra = []
        ...
        for t in range(max_len):
            decoder_state = DecoderState(feed, rnn_state, hiddens, torch.stack(all_tokens))
            ...
            new_pred[:, 0 : 4 + 1] -= 99999
            new_pred[:, 2] += 99999  # put eos back TODO single step
            pred = pred + new_pred
        ...
        return (torch.stack(all_preds), torch.stack(all_tokens), rnn_state, list(zip(*all_extra)))
    ...
    def sample(
        ...
        if self.rnntype == nn.LSTM:
            if rnn_state is None:
                rnnstate = tuple(
                    torch.zeros(self.n_layers, n_batch, self.n_hidden).to(device) for _ in range(2)
                )
            else:
        ...
        running_proj = torch.zeros(max_len + 1, n_batch, len(self.tokenizer)).to(device)
    ...
    def sample_with_gumbel(
        ...
        def token_picker(t, logits):
            ...
            if t == 0:
                toks = torch.LongTensor([sos] * n_batch).to(device)
                onehots = F.one_hot(toks, num_classes=len(self.tokenizer)).float()
                running_proj[0, range(n_batch), toks] = 1
    ...
    def forward_onehot(
        ...
        if att_features is None:
            ...
            assert len(att_features) == len(att_tokens)
            assert len(self.attention) == len(att_features) + (1 if self.self_attention else 0)
            att_masks = tuple((toks == 0).float() for toks in att_tokens)
            att_token_proj = tuple(self._make_projection(toks) for toks in att_tokens)
        ...
        for t in range(max_len):
            decoder_state = DecoderState(feed, rnn_state, hiddens, onehots)
    ...
    def logprob(self, ref_tokens, rnn_state=None, att_features=None, att_tokens=None):
        ...
        if len(ref_tokens.shape) == 3:
            ...
            preds, _, _, *_ = self.forward_onehot(
                rnn_state,
                out_src.shape[0],
                ref_tokens=out_src,
                att_features=att_features,
    ...
    def beam(
        ...
        for t in range(max_len):
            ...
            pred, feed, rnn_state, hidden, *_ = self.step(
                ...
                tuple(
                    p.expand(p.shape[0], len(beam), p.shape[2])
                    for p in att_token_proj
                    if p is not None
                ),
                self_att_proj,
            )
